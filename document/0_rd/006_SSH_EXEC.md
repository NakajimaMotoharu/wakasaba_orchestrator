...
| 依存ライブラリ  | JSch（`com.github.mwiede:jsch`）                                   |
...
- 接続失敗時（`JSchException`）は例外を吸収し `false` を返すこと。これにより再起動中のサーバへのポーリング待機が実現されること。
...
| `cmd`      | `String`                | `private final` | 実行対象のコマンド文字列                      |
...
- `isAlive()` での `JSchException` 吸収はSSH接続不可（サーバ起動中）を表す。秘密鍵読込失敗等のセッション生成エラーはこれとは異なり、上位へ伝播させること。
