...
- Pl3xMapダウンロードURLが `null` でない場合（バージョン一致の場合）、ダウンロード済みPl3xMap
  JARのSHA-512チェックサムをリモートサーバ上で計算し、Modrinth APIから取得した期待値と比較できること。
- SHA-512が一致する場合、ダウンロードファイルを本番プラグインディレクトリへ移動できること。
- SHA-512が一致しない場合、Pl3xMap JARのファイル移動をスキップすること。
...
| フィールド名 | 型                   | 修飾子                    | 要件                                  |
|--------|---------------------|------------------------|-------------------------------------|
| `log`  | `ArrayList<String>` | `private static final` | `Main.log` への参照。コマンドと標準出力のログ記録に使用する |
