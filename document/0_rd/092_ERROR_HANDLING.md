# wakasaba_orchestrator 要件定義書 補足：例外・エラーハンドリング要件

## 概要

本アプリケーションにおける例外処理の方針と、各例外の種類・発生状況・伝播経路をまとめる。

---

## 基本方針

各業務メソッドでは個別の回復処理を行わず、例外を `WksWorkFlow.execScheduledJob` から `Main.main` まで伝播させること。  
`Main.main` はワークフロー実行を `try-catch` で囲み、`Exception`
を捕捉してスタックトレースをログへ保存した後、終了時刻記録とログファイル出力を行うこと。  
ただし、`Main.outLog` が送出する `IOException` は `main` で捕捉せずJVMへ委譲すること。
...
`session.connect()` が投げるすべての `JSchException` は `isAlive()` 内で `false` として吸収すること。

これにより `shutdown` 後の再起動待ちがポーリングで実現される。  
なお、`getSessionInstance()` 内での `JSchException`（秘密鍵読込失敗等）は吸収せず上位に伝播させること。

---

## ログ未出力のケース

以下の状況ではログファイルが出力されない（または不完全になる）。これらはすべて仕様上許容される動作である。

| 状況                     | 結果                                         |
|------------------------|--------------------------------------------|
| `main` に渡す引数が4つでない     | 標準出力にUSAGEメッセージを表示して終了。ログファイルは生成されない       |
...
本要件では以下は対象外とするが、将来の改善候補として記録する。
