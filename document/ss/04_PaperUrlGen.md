...
- すべてのメソッドが `ObjectMapper.readTree()` を直接呼び出しているため、JSONパース例外（`JacksonException`
  ）が検査例外として発生し得るが、メソッドシグネチャに `throws` 宣言がない。Jackson 3.x では `JacksonException`
  が非検査例外として扱われているため問題なく動作する。
