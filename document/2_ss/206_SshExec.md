...
| 型         | 説明                                                                                    |
|-----------|---------------------------------------------------------------------------------------|
| `boolean` | `true`: SSH接続成功（サーバ生存）/ `false`: `session.connect()` が投げるすべての `JSchException` を吸収した結果 |

#### 例外

| 例外クラス           | 発生条件                          |
|-----------------|-------------------------------|
| `JSchException` | セッションインスタンスの生成失敗（秘密鍵読み込みエラー等） |

> `session.connect()` が投げるすべての `JSchException` は `false` で吸収する。`getSessionInstance()`
> が投げる `JSchException`（秘密鍵読込失敗等）は呼び出し元に伝播する。
