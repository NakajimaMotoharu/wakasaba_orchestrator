...
| 目的       | 複数のLinuxサーバに対して定型メンテナンス処理（OS更新・PaperMCサーバ更新・バックアップ等）を自動実行するオーケストレーションバッチ。実行契機は手動実行（スケジューラ等からの呼び出しを含む）      |
...
| [WksWorkFlow](./202_WksWorkFlow)                     | `com.wks.workflow` | class  | 5台のリモートサーバ＋自サーバへの処理シーケンス定義         |
