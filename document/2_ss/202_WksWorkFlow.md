...
| 種別       | class（public）                                                                      |
| 責務       | 5台のリモートサーバおよび自サーバに対する処理シーケンスを定義する。各サーバへの操作順序（update → upgrade → shutdown 等）を一元管理する |
...
| メソッド名              | 戻り値型   | 修飾子             | 説明                                 |
|--------------------|--------|-----------------|------------------------------------|
| `execScheduledJob` | `void` | `public static` | 6つの対象（リモート5台＋自サーバ）に対して定型処理を順番に実行する |
...
| 引数名       | 型          | 説明                                                         |
|-----------|------------|------------------------------------------------------------|
| `servers` | `String[]` | 長さ5のサーバ接続情報ファイルパス配列。`servers[0]`〜`servers[4]` がそれぞれのサーバに対応 |
