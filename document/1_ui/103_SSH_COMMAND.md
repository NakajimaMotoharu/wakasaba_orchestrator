...
| `isAlive`             | `boolean` | `public static`  | 対象サーバへ SSH 接続可能か判定する                        |
...
処理開始時点で対象サーバへSSH接続可能か判定する。コマンド実行および標準出力ログの追加は行わない。
...
- `movePaperMc` は `runCommand` を経由せず、`SshExec` を直接操作してコマンドを実行する。SHA検証の結果によってコマンドの実行有無を制御する必要があるため、共通処理である
